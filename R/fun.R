...
prodXY <- function(X, Y, sdX, sdY, covXY = 0) {
  eXY <- X * Y + covXY
  varXY <-
    X ^ 2 * sdY^2 + Y ^ 2 * sdX^2 + 2 * X * Y * covXY + sdX^2 * sdY^2 + covXY ^
    2
  return(list("mean" = eXY, "sd" = varXY))
}
...
divXY <- function(X, Y, sdX, sdY, covXY = 0) {
  ...
  sdXY <-
    sdX^2 / Y ^ 2 - 2 * X * covXY / Y ^ 3 + X ^ 2 * sdY^2 / Y ^ 4
...
logX <- function(X, sdX) {
  ...
  sd.logX <- sdX^2 / X ^ 2
  return(list("mean" = E.logX, "sd" = var.logX))
}
